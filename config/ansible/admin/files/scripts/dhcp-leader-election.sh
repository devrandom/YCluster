...
# Endpoints come from /etc/ycluster/etcd-client.env (ETCDCTL_ENDPOINTS), which
# the systemd unit injects via EnvironmentFile=; that same file also exports
# ETCDCTL_CACERT/CERT/KEY when TLS is on, which etcdctl picks up automatically.
# A positional arg still overrides for manual runs.
