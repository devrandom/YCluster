...
| 2026-06-11 | VM usage/scheduling/accounts — all 7 steps of `docs/design/vm-usage-scheduling-accounts.md` (authentik IdP + invitations + `ycluster user`, OWUI OIDC cutover, forward-auth, event log + sampling, dashboard, scheduler, quotas) | Operator: **1 day**. Agent doc estimate: 11–19 d; after the house ~5× correction, widened for the earned unknowns (authentik flow iteration against live GitHub OAuth, OWUI account-merge verification): **2.5–4 d** | open — done when all 7 steps run on the real cluster, idempotent, invitation flow exercised by a real user, one scheduled stop observed graceful. OWUI soak is calendar time and doesn't count; setting it up does | — |
