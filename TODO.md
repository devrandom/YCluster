...
- **"core" definition consistency — residual cleanup.** The 2026-06-10 audit found three meanings of "core" and the two substantive ones are now resolved:
  ...
  - RESIDUAL (low severity): (1) `dhcp_server.py:48` `CORE_NODE_IPS` hardcodes s1-s3 → IP as a *fallback* used only when netplan parsing fails (s4+ still get their convention IP via the normal path, verified live); generalize it to derive from the `sN -> 10.0.0.10+N` convention. (2) Stale "s1-s3" comments now that core is dynamic: `group_vars/core.yml:1`, `setup-etcd-tls.yml:206`. (3) Optional: have the plugin warn if live etcd membership disagrees with the static `etcd` group (drift cross-check).
